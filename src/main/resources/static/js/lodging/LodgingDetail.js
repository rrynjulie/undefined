...
function fetchBookingsByRoomId(roomId) {
    fetch(`lodging/LodgingDetail/${roomId}`)
        ...
        .then(data => {
            ...
            checkAvailability(); // 예약 가능 여부를 확인하는 함수 호출
        })
...
function checkAvailability() {
    ...
    if (overlappingBooking) {
        // 예약 불가능 처리
        document.getElementById('message').innerText = '이미 예약된 기간입니다. 다른 날짜를 선택해주세요.';
    } else {
        // 예약 가능 처리
        document.getElementById('message').innerText = '예약 가능합니다.';
        // 예약을 진행하는 로직 추가
        // 예약을 진행할 때는 서버로 데이터를 전송하여 예약을 등록하거나 화면을 전환하는 등의 작업을 수행할 수 있습니다.
    }
}

// 예약 가능 여부 확인 버튼 클릭 이벤트 리스너 추가
...
checkAvailabilityButton.addEventListener('click', function(event) {
    ...
    fetchBookingsByRoomId(document.getElementById('roomId').value); // 선택된 객실의 예약 정보를 가져옴
});
