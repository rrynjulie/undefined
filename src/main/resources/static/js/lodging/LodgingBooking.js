...
//         // const storedStartDate = sessionStorage.getItem('startDate');
//         // const storedEndDate = sessionStorage.getItem('endDate');
//         const storedStartDate = (sessionStorage.getItem('startDate')).toISOString().split('T')[0];
//         const storedEndDate =(sessionStorage.getItem('endDate')).toISOString().split('T')[0];
//
//         const storedAdultCount = parseInt(sessionStorage.getItem('adultCount')) || 1;
//         const storedChildCount = parseInt(sessionStorage.getItem('childCount')) || 0;
//
...
//
//         if (storedStartDate && storedEndDate) {
//             document.getElementById('bookingStartDate').value = storedStartDate.split('T')[0];
//             document.getElementById('bookingEndDate').value = storedEndDate.split('T')[0];
//         }
//
//         // 폼에 날짜 정보 추가
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function showBookingInfo() {
        ...
        if (storedStartDate && storedEndDate) {
            // 시간 정보 제거 후 문자열 생성
            const startDateStr = storedStartDate.split('T')[0];
            const endDateStr = storedEndDate.split('T')[0];
    ...
    bookingForm.addEventListener('submit', function (event) {
        ...
        if (storedStartDate && storedEndDate) {
            ...
            hiddenStartDateField.type = 'hidden';
            hiddenStartDateField.name = 'bookingStartDate';
            hiddenStartDateField.value = storedStartDate.split('T')[0]; // 시간 정보 제거
            bookingForm.appendChild(hiddenStartDateField);

            const hiddenEndDateField = document.createElement('input');
            hiddenEndDateField.type = 'hidden';
            hiddenEndDateField.name = 'bookingEndDate';
            hiddenEndDateField.value = storedEndDate.split('T')[0]; // 시간 정보 제거
            bookingForm.appendChild(hiddenEndDateField);
        }
    });
});
